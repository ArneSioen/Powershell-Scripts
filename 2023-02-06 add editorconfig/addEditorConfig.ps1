...
foreach ($project in $json_projects.value) {
    ...
    foreach ($repo in $json_repos) {     
        ...
        Copy-Item -Path "C:\Users\amoer\source\repos\personal\Tools\PowerShell Scripts\2023-02-06 add editorconfig\.editorconfig" -Destination "."
        $gitStatus = git status
        ...
        Copy-Item -Path "C:\Users\amoer\source\repos\personal\Tools\PowerShell Scripts\2023-02-06 add editorconfig\.editorconfig" -Destination "."
        git add --all
        git commit -m "add .editorconfig to repo"
        git push
